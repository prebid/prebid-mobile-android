...
public class PrebidInitTest extends BaseSetup {
    ...
    @Test
    public void testInitWithNullPrebidHost() {
        ArrayList<AdUnit> adUnits = new ArrayList<AdUnit>();
        BannerAdUnit adUnit1 = new BannerAdUnit(TestConstants.bannerAdUnit1, TestConstants.configID1);
        adUnit1.addSize(320, 50);
        adUnits.add(adUnit1);
        try {
            Prebid.init(activity.getApplicationContext(), adUnits, TestConstants.accountId, null);
        } catch (Exception e) {
            Assert.assertEquals(PrebidException.PrebidError.NULL_PREBID_HOST.getDetailMessage(), e.getMessage());
        }
    }
