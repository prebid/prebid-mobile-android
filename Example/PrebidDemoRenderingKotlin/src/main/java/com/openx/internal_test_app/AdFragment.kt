...
import com.openx.apollo.models.openrtb.bidRequests.assets.NativeAssetTitle
import com.openx.apollo.sdk.ApolloSettings
import com.openx.internal_test_app.plugplay.config.*
import com.openx.internal_test_app.utils.*
...
abstract class AdFragment : BaseFragment() {
    ...
    protected var refreshDelay = ApolloSettings.AUTO_REFRESH_DELAY_DEFAULT / 1000
    ...
    override fun initUi(view: View, savedInstanceState: Bundle?) {
        arguments?.let {
            ...
            val title = it.getString(getString(R.string.key_title), getString(R.string.segment_title_apollo))
            setTitle(title)
            shouldSetNoBids()
        }
    ...
    private fun setNoBidsAccountId(enable: Boolean) {
        if (enable) {
            ApolloSettings.setAccountId(getString(R.string.openx_account_id_prod_no_bids))
        }
        else {
    ...
    private fun isNoBids(): Boolean = configId == getString(R.string.apollo_no_bids_config_id)
